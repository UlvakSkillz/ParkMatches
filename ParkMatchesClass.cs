...
public static class Patch
{
    ...
    {
        if ((ParkMatches.ParkMatchesClass.instance.playerManager.AllPlayers.Count >= 2) && (!ParkMatches.ParkMatchesClass.instance.fightStarted))
        {
            int lowestActor = 256;
            //for all players, get lowest actor number for clients
            foreach (Player tempPlayer in ParkMatches.ParkMatchesClass.instance.playerManager.AllPlayers)
            {
                //if host found
            ...
            ParkMatches.ParkMatchesClass.instance.clientActorNumber = (short)lowestActor;
            //if host or lowest actor number
            if ((ParkMatches.ParkMatchesClass.instance.localPlayer.Data.GeneralData.ActorNo == 1) || (ParkMatches.ParkMatchesClass.instance.localPlayer.Data.GeneralData.ActorNo == lowestActor))
            {
                //start match
...
namespace ParkMatches
{
    public class ParkMatchesClass : MelonMod
    {
        ...
        //updates the scoreboard to reflect match scores
        public void updateScoreboard()
        {
            parkMatchCounter.firstPlayer = hostPlayer;
            parkMatchCounter.secondPlayer = clientPlayer;
            parkMatchCounter.RPC_SyncMatchCounterPlayers(1, clientActorNumber);
            parkMatchCounter.RPC_HardResetScores();
        }
        ...
        public void startFight()
        {
            try
            {
                ...
                //start fight
                resetPlayers();
                localDeathTimer = DateTime.Now.AddSeconds(3);
                fightStarted = true;
                //increase ring size
